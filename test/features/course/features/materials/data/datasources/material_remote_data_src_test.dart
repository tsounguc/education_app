...
import 'package:education_app/features/course/features/materials/data/models/resource_model.dart';
...
void main() {
  ...
  group('addMaterial', () {
    setUp(() async {
      await firestore.collection('courses').doc(tResource.courseId).set(CourseModel.empty().toMap());
    });
    test('should add the provided [Material] to the firestore', () async {
      await remoteDataSource.addMaterial(tResource);

      final collectionRef =
          await firestore.collection('courses').doc(tResource.courseId).collection('materials').get();
