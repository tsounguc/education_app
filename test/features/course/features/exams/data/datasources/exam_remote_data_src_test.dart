...
import 'package:education_app/features/course/features/exams/data/models/exam_model.dart';
import 'package:education_app/features/course/features/exams/data/models/exam_question_model.dart';
import 'package:education_app/features/course/features/exams/data/models/user_choice_model.dart';
import 'package:education_app/features/course/features/exams/data/models/user_exam_model.dart';
...
void main() {
  ...
  group('uploadExam', () {
    test(
      ...
      () async {
        ...
        // Assert
        final examDocs = await firestore.collection('courses').doc(exam.courseId).collection('exams').get();
        ...
        final questionModel = ExamQuestionModel.fromMap(questionDocs.docs.first.data());
  ...
  group('getExamQuestions', () {
    test('should return the questions of the given exam', () async {
      ...
      final examsCollection = await firestore.collection('courses').doc(exam.courseId).collection('exams').get();
  ...
  group('updateExam', () {
    test('should update the given exam', () async {
      ...
      // assert
      final updatedExam =
          await firestore.collection('courses').doc(exam.courseId).collection('exams').doc(examModel.id).get();
    ...
    test(
      ...
      () async {
        ...
        await firestore.collection('users').doc(auth.currentUser!.uid).set(
              const LocalUserModel.empty().copyWith(uid: auth.currentUser!.uid, points: 1).toMap(),
            );
        // Act
  ...
  group('getUserExams', () {
    test('should return the exams of the current user', () async {
      ...
      await firestore.collection('users').doc(auth.currentUser!.uid).collection('courses').doc(exam.courseId).set(
            CourseModel.empty().copyWith(id: exam.courseId).toMap(),
          );
      await remoteDataSource.submitExam(exam);
