...
import 'package:education_app/features/course/features/exams/data/models/question_choice_model.dart';
...
class ExamRemoteDataSrcImpl implements ExamRemoteDataSrc {
  ...
  Future<List<ExamQuestionModel>> getExamQuestions(Exam exam) async {
    try {
      ...
      final result = await _firestore
          ...
          .get();
      return result.docs.map((doc) => ExamQuestionModel.fromMap(doc.data())).toList();
  ...
  Future<List<UserExamModel>> getUserCourseExams(String courseId) async {
    try {
      ...
      final result = await _firestore
          ...
          .collection('exams')
          .get();
      return result.docs.map((doc) => UserExamModel.fromMap(doc.data())).toList();
    } on FirebaseException catch (e) {
      throw ServerException(
        message: e.message ?? 'Unknown error',
        statusCode: e.code,
  ...
  Future<void> submitExam(UserExam exam) async {
    try {
      ...
      final totalPoints =
          exam.answers.where((answer) => answer.isCorrect).fold<int>(0, (previousValue, _) => previousValue + 1);

      final pointPercent = totalPoints / exam.totalQuestions;
      final points = pointPercent * 100;

      await _firestore.collection('users').doc(user.uid).update({'points': FieldValue.increment(points)});
